...
function insertProducts(){
    ...
    if(isset($_SERVER["REQUEST_METHOD"]) == "POST"){
        if (isset($_POST['btn-save-products'])) {
            ...
            if($row->num_rows > 0){
                $_SESSION['staff-status'] = "Error";
                $_SESSION['staff-code'] ="error";
                $_SESSION['staff-message'] = "Product already exist.";
             ...
             //insert pizza if selected
                ...
                foreach($pizzaSize as $key => $value){
                    ...
                    if ($insertProduct) {
                        $_SESSION['staff-status'] = "Successful";
                        $_SESSION['staff-code'] ="success";
                        $_SESSION['staff-message'] = "Insert new product successfully";
                        header('Location:products.php');
                    } else{
                        $_SESSION['staff-status'] = "Error";
                        $_SESSION['staff-code'] ="error";
                        $_SESSION['staff-message'] = "Could not insert product";
                        header('Location:products.php');
                    }
                }
            }
        ...
        if (isset($_POST['btn-update-products'])) {
            ...
            if  ($editImageProduct  != '') {
                ...
                if ($updateProduct) {
                    ...
                    $_SESSION['staff-status'] = "Successful";
                    $_SESSION['staff-status'] ="success";
                    $_SESSION['staff-message'] = "Update product successfully";
                    header('Location:products.php');
                } else{
                    $_SESSION['staff-status'] = "Error";
                    $_SESSION['staff-status'] ="error";
                    $_SESSION['staff-message'] = "Could not update product";
                    header('Location:products.php');
                }
            } else {
                //check image product
    ...
    if(isset($_SERVER["REQUEST_METHOD"]) == "POST"){
        if (isset($_POST['btn-delete-products'])) {
            ...
            if ($alter) {
                $_SESSION['staff-status'] = "Successful";
                $_SESSION['staff-code'] ="success";
                $_SESSION['staff-message'] = "Delete product successfully";   
                header('Location:products.php?');
            }  else{
                $_SESSION['staff-status'] = "Error";
                $_SESSION['staff-code'] ="error";
                $_SESSION['staff-message'] = "Could not delete item";
                header('Location:products.php');
            }
        }
    }
